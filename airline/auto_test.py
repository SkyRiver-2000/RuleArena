...
from micro_evaluation import check_rule_application, compute_metrics, aggregate_rule_wise_metrics
...
def eval_query(llm, question_prompt: str, info_dict: dict, query_idx: int, args):
    error_cnt = 0
    rule_file = "reference_rules.txt" if not args.textual else "reference_rules_textual.txt"
    ...
    while True:
        try:
            ...
            if start_idx != -1:
                ...
                if value_idx != -1:
                    value = conclusion[value_idx+1:].replace(",", "").replace(".", "")
                    if value.isnumeric():
                        value = int(value)
                    pred = value
            ...
            rule_app_checklist, structured_response = \
                check_rule_application(question_prompt, info_dict, response)
            break
