...
us_city_collection = [
    "Seattle", "Portland", "Los Angeles", "San Francisco", "Sacramento",
    "Salt Lake City", "Denver", "Las Vegas", "Phoenix",
    "Dallas", "Houston", "Austin", "Minneapolis", "New Orleans",
    "Chicago", "Charlotte", "Washington D.C.", "Philadelphia", "New York",
    "Boston", "Atlanta", "Orlando", "Miami",
]
...
customer_types = ["Basic Economy", "Main Cabin", "Main Plus", "Premium Economy", "Business", "First"]
name_collection = [
    "James", "John", "William", "Robert", "Michael",
...
def gen_question(complexity: int):
    ...
    prompt_bag_list.append(f"1. A backpack: {length} x {width} x {height} inches, {weight} lbs;")
    ...
    for bag_id in range(1, num_bags):
        ...
        if overweight:
            if region in ["Cuba", "Europe", "Israel", "Qatar", "Australia", "New Zealand"]:
        ...
        prompt_bag_list.append(f"{bag_id + 1}. A {bag_class}: {length} x {width} x {height} inches, {weight} lbs;")
