    ...
    def set_reference_overweight(self, info_dict):
        ...
        for bag in self.checked_bags:
            bag.__dict__["reference_overweight"] = compute_overweight(
                {"weight": bag.weight}, info_dict["routine"], info_dict["customer_class"], False
            )
...
def check_basic_travel_info(response: Response, reference: Response):
    ...
    if departure != reference.place_of_departure or \
        arrival != reference.place_of_arrival:
        return True
...
def check_base_bag_payment(response: Response, reference: Response):
    ...
    elif reference.passenger_class == "Main Plus" and (pred_num_free == ref_num_free - 1):
...
def check_overweight_and_oversize(response: Response, reference: Response):
    ...
    for pred_bag, ref_bag in zip(response.checked_bags, reference.checked_bags):
        ...
        if pred_bag.overweight_fee != ref_bag.overweight_fee:
            if ref_bag.weight > 50 and ref_bag.weight <= 70 and \
                ref_bag.overweight_fee == 0 and \
                pred_bag.overweight_fee == ref_bag.reference_overweight:
            ...
            if ref_bag.weight > 50 and ref_bag.weight <= 70 and \
                ref_bag.overweight_fee == 0:
                examine_list.append("Correct: complementary overweight")
            examine_list.append("Correct: overweight fee matching")
        
        if pred_bag.total_fee != ref_bag.total_fee and \
            ref_bag.overweight_fee > 0 and ref_bag.oversize_fee > 0:
            if pred_bag.total_fee == pred_bag.base_check_fee + pred_bag.oversize_fee + pred_bag.overweight_fee:
                examine_list.append("Missing: maximum violation fee")
            elif pred_bag.total_fee == pred_bag.base_check_fee + max(pred_bag.oversize_fee, pred_bag.overweight_fee):
...
def error_analysis(response: Response, question_info_dict: dict):
    ...
    override = {"check_base": override_check_base} if len(base_check_fee_judgement) else {}
