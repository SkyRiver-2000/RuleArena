...
from micro_evaluation import rule_list, compute_answer, analyze_response, compute_metrics, aggregate_rule_wise_metrics
...
def eval_query(llm, tax_payer: dict, tax_payer_pydantic: TaxPayer, args, query_idx: int):
    error_cnt = 0
    while True:
        try:
            error_cnt += 1
            ...
            if match:
                ...
                if np.isclose(value, answer) or (isinstance(value, str) and str(answer) in value):
                    acc = True

            rule_applications, structured_response = analyze_response(response, tax_payer, tax_payer_pydantic)
            break
...
parser.add_argument("--distractor", action="store_true")                    # Whether to add distractive rules (NOTE: only use when complexity = 0)
parser.add_argument("--placeholder", action="store_true")                   # Whether to add meaningless tokens (NOTE: only use when complexity = 0)
parser.add_argument("--chunk_size", type=int, default=1)                    # Number of API Request sent in parallel
parser.add_argument("--log_dir", type=str, default="logs")                  # Directory to store the experiment logs
parser.add_argument("--remake_data", action="store_true")                   # Re-create synthesized data
parser.add_argument("--start_idx", type=int, default=0)                     # Resume experiments from a given question id
...
if args.llm.startswith("o1"):
    ...
    vertexai.init(project=PROJECT_ID, location=MODEL_LOCATION, staging_bucket=BUCKET_URI)
...
log_path = f"{args.llm}_comp_{args.complexity}_1shot" if args.use_example else f"{args.llm}_comp_{args.complexity}"
if args.textual:
    log_path += "_textual"
if args.distractor:
...
if args.remake_data or not os.path.exists(problem_file):
    problem_set = []
    for _ in range(100):
        tax_payer_dict, tax_payer_pydantic = generate_taxpayer(complexity=args.complexity)
        problem_set.append({"dict": tax_payer_dict, "pydantic": tax_payer_pydantic.model_dump()})
...
if args.start_idx >= 1:
    for i in range(args.start_idx):
        tax_payer, tax_payer_pydantic = problems[i]["dict"], TaxPayer(**problems[i]["pydantic"])
        ...
        for j in range(len(lines)):
            if lines[j].startswith("Rule Applications:"):
                break
        if j != len(lines) and any([lines[j+1].startswith(tag) \
    ...
    for i in range(args.start_idx, len(problems), args.chunk_size):
        ...
        with ThreadPoolExecutor() as executor:
            iterator = executor.map(
                lambda p: eval_query(llm, p[1]["dict"], TaxPayer(**p[1]["pydantic"]), args, p[0]),
...
for rule_app_checklist in rule_applications:
    if len(rule_app_checklist) == 0:
        continue
    problem_wise_binary, problem_wise_ratio, rule_wise = compute_metrics(rule_app_checklist)
...
rule_wise_recall, rule_wise_precision, rule_wise_total = aggregate_rule_wise_metrics(rule_wise_app_list)
