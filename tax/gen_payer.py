...
def generate_sche_3(status_dict: dict):
    ...
    education_credits = "$TBD" if status_dict["has_student_loans_or_education_expenses"] else 0
...
def generate_f8863():
    ...
    for _ in range(num_students):
        ...
        prompt = prompt.replace("$qualified_student_expenses", "$"+f"{qualified_student_expenses:,}")
...
def generate_basic_f1040(status_dict: dict):
    ...
    qualified_dividends = np.random.randint(1000, 5000) if status_dict["has_qualified_dividends"] else 0
    ...
    taxable_social_security_benefits = round(social_security_benefits * random.uniform(0, 0.85))
    ...
    american_opportunity_credit = "$TBD" if status_dict["has_student_loans_or_education_expenses"] else 0
...
def generate_taxpayer(complexity: int):
    ...
    if not student:
        filing_status = random.choice([
            "single", "married filing separately", "qualifying surviving spouse", "married filing jointly", "head of household"
        ])
    else:
